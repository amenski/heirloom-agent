import type { PromptBufferState } from "./prompt-buffer.js";

export interface PromptUndoRedoState {
...
export function recordPromptEdit(
  ...
  if (next.text === current.text || next.text === history.undoStack.at(-1)?.text) return;
  history.undoStack.push(current);
  if (history.undoStack.length > maxUndoEntries) history.undoStack = history.undoStack.slice(-maxUndoEntries);
  history.redoStack = [];
}
